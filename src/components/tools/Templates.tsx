...
const TemplatePreview: React.FC<{ template: Template; data: Record<string, string>; onClose: () => void; saved: boolean; onSave: () => void }> = ({ template, data, onClose, saved, onSave }) => {
    ...
    const handlePrint = () => {
        // Render into #document-print-root so print CSS shows it
        ...
        el.innerHTML = `
            <div style="max-width:700px;margin:0 auto;padding:40px 50px 60px;font-family:Georgia,'Times New Roman',serif;color:#1a1a1a;line-height:1.7;font-size:11pt;">
                <div style="text-align:center;margin-bottom:40px;border-bottom:2px solid #1a1a1a;padding-bottom:24px;">
                    <div style="font-size:10pt;letter-spacing:3px;text-transform:uppercase;color:#666;margin-bottom:8px;">Ready Legacy — Document</div>
                    <h1 style="font-size:22pt;margin:0 0 8px 0;font-weight:700;">${template.title}</h1>
                    <div style="font-size:10pt;color:#666;">${template.subtitle || ''} · ${new Date().toLocaleDateString('de-CH')}</div>
                </div>
                ${template.steps.map(step => `
                    <div style="margin-bottom:24px;">
                        <h4 style="font-size:9pt;text-transform:uppercase;letter-spacing:0.5px;color:#444;margin-bottom:12px;">${step.title}</h4>
                        ${step.fields.filter(f => data[f.key]).map(field => `
                            <div style="display:flex;gap:12px;margin-bottom:10px;">
                                <span style="font-weight:700;font-size:9pt;text-transform:uppercase;letter-spacing:0.5px;color:#444;min-width:200px;">${field.label}</span>
                                <span style="flex:1;border-bottom:1px solid #ddd;padding-bottom:4px;">${data[field.key]}</span>
                            </div>
                        `).join('')}
                    </div>
                `).join('')}
                <div style="margin-top:60px;border-top:1px solid #ccc;padding-top:30px;display:flex;justify-content:space-between;gap:40px;">
                    <div style="flex:1;"><div style="border-bottom:1px solid #1a1a1a;height:60px;"></div><div style="font-size:9pt;color:#666;margin-top:6px;">Place, Date</div></div>
                    <div style="flex:1;"><div style="border-bottom:1px solid #1a1a1a;height:60px;"></div><div style="font-size:9pt;color:#666;margin-top:6px;">Signature</div></div>
                </div>
                <div style="margin-top:40px;padding:16px;border:1px solid #ddd;font-size:8pt;color:#888;line-height:1.5;">
                    This document was generated by Ready Legacy (readylegacy.ch) and is intended as a draft for organizational purposes only.
                    It does not constitute legal advice. Please consult a qualified notary or lawyer in your jurisdiction.
                </div>
                <div style="margin-top:24px;text-align:center;font-size:8pt;color:#aaa;">readylegacy.ch · Swiss Made · GDPR &amp; nDSG Compliant</div>
            </div>
        `;
        window.print();
        el.innerHTML = '';
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ padding: '16px 24px', borderTop: '1px solid var(--glass-border)', display: 'flex', gap: '10px', justifyContent: 'flex-end', alignItems: 'center' }}>
                    ...
                    <button onClick={handlePrint} style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid var(--glass-border)', background: 'transparent', color: 'var(--text-color)', cursor: 'pointer', fontSize: '0.85rem' }}>
